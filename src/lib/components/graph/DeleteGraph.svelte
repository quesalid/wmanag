...
export let deleteQuery = (ev:any|undefined)=>console.log("DELETE DATA")
export let id: string|any = 'defaultDeleteGraphContainer'
...
<div class="data-menu" id="{'dragable'+id}">
	<header id="{'dragzone'+id}">
		<div class="data-menu-header" style="--background-color:{inpuHeaderColor}">
			<span style="margin-right: 20px;">DELETE GRAPH </span>
			<input type="button" value="CLOSE" on:click={closeMenu} />
		</div>
		 <div class="data-menu-toolbar">
			<input type="submit" on:click={deleteQuery} value="DELETE">
		</div>
